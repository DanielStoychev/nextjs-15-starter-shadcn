...
export async function GET(request: NextRequest) {
    try {
        ...
        const totalRevenue = entriesWithRevenue.reduce((sum, entry) => sum + entry.gameInstance.entryFee, 0); // Get recent entries for activity feed
    ...
    `) as any[];
        ...
        // Use Order table for user activity since User doesn't have createdAt
        const orderActivity = (await prisma.$queryRaw`
      SELECT DATE(o."createdAt") as date, COUNT(*)::int as count
      FROM "Order" o
      WHERE o."createdAt" >= ${startDate}
      GROUP BY DATE(o."createdAt")
      ORDER BY DATE(o."createdAt") ASC
    `) as any[]; // Calculate growth percentages
        ...
        const response = {
            ...
            charts: {
                ...
                userGrowth: orderActivity.map((entry: any) => ({
                    date: entry.date.toISOString().split('T')[0],
