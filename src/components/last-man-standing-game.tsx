...
export function LastManStandingGame({
    ...
    const handlePickTeam = (teamId: number) => {
        ...
        console.log('Selected Team ID:', teamId); // Log selected team
        alert(`Team ${teamId} selected!`); // Add alert for debugging
    };
                        ...
                        <div key={fixture.id} className='rounded-md border p-4'>
                            ...
                            <div className='mt-4 flex justify-around'>
                                {fixture.participants.map((participant) => (
                                    <div
                                        key={participant.id}
                                        onClick={() => handlePickTeam(participant.id)}
                                        className={`flex h-auto cursor-pointer flex-col items-center rounded-md border p-2 ${selectedTeamId === participant.id ? 'bg-primary text-primary-foreground' : 'bg-background text-foreground hover:bg-accent'}`}>
