    ...
    useEffect(() => {
        const confirmPayment = async () => {
            ...
            if (success === 'true' && sessionId && userEntry?.status === 'PENDING_PAYMENT') {
                setIsLoading(true);
                try {
                    const response = await fetch('/api/games/entry/confirm-payment', {
                        method: 'POST',
                        headers: {
                        ...
                        <CardContent className='space-y-3'>
                            ...
                            <div className='rounded-lg border border-blue-200 bg-blue-50 p-3'>
                                <p className='text-sm text-blue-800'>
                                    🎮 Your game interface is now available below! Scroll down to start making your
                                    picks and choices.
                                </p>
                            </div>
                        </CardContent>
            ...
            <CardContent className='space-y-4'>
                ...
                <div className='rounded-lg border border-amber-200 bg-amber-50 p-3'>
                    <p className='text-sm text-amber-800'>
                        💰 Pay the entry fee to join the game and unlock the playing interface where you can make your
                        picks!
                    </p>
                </div>
                <Button onClick={handlePayment} disabled={isLoading} className='w-full' size='lg'>
