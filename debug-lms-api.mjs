...
// Import the SportMonks API functions
import { getLeagueDetails, getRoundFixtures, getSeasonRounds } from './src/lib/sportmonks-api.ts';
...
async function debugLMSRoundSelection() {
    try {
        console.log('🔍 Debugging LMS Round Selection with API...');
        console.log('Current date:', new Date().toISOString());

        // Get Premier League details
        console.log('\n📊 Fetching Premier League details from SportMonks API...');
        ...
        if (!currentSeasonData) {
            console.log('❌ No current season found from API');

            return;
        }

        if (currentSeasonData.finished) {
            console.log('⚠️  Current season is finished according to API');

            return;
        }

        // Get rounds for the season
        console.log('\n🔄 Fetching season rounds from SportMonks API...');
        const roundsResponse = await getSeasonRounds(currentSeasonData.id);
        const rounds = roundsResponse.data || [];

        console.log(`Found ${rounds.length} rounds from API:`);
        ...
        if (targetRoundData) {
            ...
            console.log(`Found ${initialFixturesData.length} fixtures from API`);
