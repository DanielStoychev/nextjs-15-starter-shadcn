...
async function checkDatabaseState() {
  ...
  try {
    ...
    users.forEach(user => {
      console.log(`   - ${user.email} (${user.username}) - ${user.role} - ${user.emailVerified ? 'Verified' : 'Unverified'}`);
    });
    ...
    instances.forEach(instance => {
      console.log(`   - ${instance.game.name}: ${instance.name} - £${(instance.entryFee / 100).toFixed(2)} - ${instance.status}`);
    ...
    entries.forEach(entry => {
      console.log(`   - ${entry.user.email} in ${entry.gameInstance.game.name}: ${entry.gameInstance.name} - ${entry.status}`);
    });
